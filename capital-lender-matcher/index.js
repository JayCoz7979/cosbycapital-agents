...
${lenders.slice(0, 30).map((l, i) =>
  `${i + 1}. ${l.name} | Type: ${l.type} | Min: $${l.loan_min?.toLocaleString()} - Max: $${l.loan_max?.toLocaleString()} | Min Credit: ${l.min_credit} | Min Revenue: $${l.min_revenue?.toLocaleString()} | Min Months: ${l.min_time_months} | States: ${l.states?.join(", ")}`
).join("\n")}
...
async function sendLenderMatchEmail(profile, matches, lenders) {
  ...
  const matchesHtml = topMatches.map(m => {
    ...
    <div style="border:1px solid #e5e7eb;border-radius:8px;padding:16px;margin-bottom:12px;">
      ...
      <p style="margin:0 0 8px;color:#6b7280;font-size:12px;">${lender.type} • Up to $${lender.loan_max?.toLocaleString()} • ${m.estimated_rate}</p>
