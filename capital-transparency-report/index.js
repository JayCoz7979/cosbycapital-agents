...
async function buildTransparencyReport(profile, data) {
  ...
  const lenderRows = lenderMatches.slice(0, 10).map(m => `
    <tr>
      ...
      <td style="padding:10px 8px;border-bottom:1px solid #f3f4f6;font-size:12px;color:#6b7280;">${m.lenders?.lender_type || "—"}</td>
      <td style="padding:10px 8px;border-bottom:1px solid #f3f4f6;text-align:center;">
    ...
    for (const profile of profiles) {
      try {
        ...
        const [grantMatchRes, lenderMatchRes, appRes] = await Promise.all([
          ...
          supabase.from("lender_matches").select("*, lenders(name, lender_type, max_loan)").eq("profile_id", profile.id).order("match_score", { ascending: false }),
